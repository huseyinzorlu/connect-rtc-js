...
export default class StandardStrategy extends CCPInitiationStrategyInterface {
  ...
  _isEarlyMediaConnectionSupported() {
    return (
      isChromeBrowser() && getChromeBrowserVersion() >= CHROME_SUPPORTED_VERSION
    );
  }

  _createRtcPeerConnection(
    rtcPeerConnectionConfig,
    rtcPeerConnectionOptionalConfig
  ) {
  ...
  setRemoteDescription(self, rtcSession) {
    var setRemoteDescriptionPromise = rtcSession._pc.setRemoteDescription(
      self._createSessionDescription({
        type: "answer",
        sdp: self._sdp,
      })
    );
    ...
    setRemoteDescriptionPromise
      .then(() => {
        var remoteCandidatePromises = Promise.all(
          self._candidates.map(function (candidate) {
      ...
      .catch(() => {
        ...
        self.transit(
          new FailedState(rtcSession, RTC_ERRORS.SET_REMOTE_DESCRIPTION_FAILURE)
        );
      });
  }
  ...
  _ontrack(self, evt) {
    ...
    if (evt.track.kind === "video" && self._remoteVideoElement) {
      ...
      window.mainRemoteStreamFromAws = evt.streams[0];
