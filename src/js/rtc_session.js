...
export class RTCSessionState {
  ...
  onRemoteHungup() {
    throw new UnsupportedOperation(
      "onRemoteHungup not implemented by " + this.name
    );
  }
  get name() {
  ...
  onSignalingConnected() {
    throw new UnsupportedOperation(
      "onSignalingConnected not implemented by " + this.name
    );
  }
  onSignalingHandshaked() {
    throw new UnsupportedOperation(
      "onSignalingHandshaked not implemented by " + this.name
    );
  }
  onSignalingFailed(e) {
    // eslint-disable-line no-unused-vars
    throw new UnsupportedOperation(
      "onSignalingFailed not implemented by " + this.name
    );
  }
...
export class GrabLocalMediaState extends RTCSessionState {
  onEnter() {
    ...
    if (self._rtcSession._isUserProvidedStream) {
      ...
      var sessionGumPromise = self._gUM(
        self._rtcSession._buildMediaConstraints()
      );

      Promise.race([sessionGumPromise, gumTimeoutPromise])
        .then((stream) => {
          self._rtcSession._sessionReport.gumTimeMillis =
            Date.now() - startTime;
  ...
  onEnter() {
    ...
    self._rtcSession._pc
      ...
      .catch((e) => {
        ...
        self.transit(
          new FailedState(self._rtcSession, RTC_ERRORS.CREATE_OFFER_FAILURE)
        );
      });
  }
  get name() {
...
export class SetLocalSessionDescriptionState extends RTCSessionState {
  onEnter() {
    ...
    self._rtcSession._pc
      .setLocalDescription(self._rtcSession._localSessionDescription)
      .then(() => {
        var initializationTime =
          Date.now() - self._rtcSession._connectTimeStamp;
        self._rtcSession._sessionReport.initializationTimeMillis =
          initializationTime;
        self._rtcSession._onSessionInitialized(
          self._rtcSession,
          initializationTime
        );
        self._rtcSession._sessionReport.setLocalDescriptionFailure = false;
        self.transit(
          new ConnectSignalingAndIceCollectionState(
            self._rtcSession,
            transformedSdp.mLines
          )
        );
      })
      .catch((e) => {
        ...
        self.transit(
          new FailedState(
            self._rtcSession,
            RTC_ERRORS.SET_LOCAL_DESCRIPTION_FAILURE
          )
        );
      });
  }
...
export class ConnectSignalingAndIceCollectionState extends RTCSessionState {
  ...
  onSignalingConnected() {
    ...
    this._rtcSession._sessionReport.signallingConnectTimeMillis =
      this._rtcSession._signallingConnectTimestamp - this._startTime;
  ...
  onSignalingFailed(e) {
    ...
    this.transit(
      new FailedState(
        this._rtcSession,
        RTC_ERRORS.SIGNALLING_CONNECTION_FAILURE
      )
    );
  }
  ...
  _checkCandidatesSufficient(candidate) {
    ...
    if (
      ...
      candidateMLineIndex < this._mLines
    ) {
      var mIndexList =
        this._iceCandidateFoundationsMap[candidateFoundation] || [];
  ...
  _reportIceCompleted(isTimeout) {
    this._rtcSession._sessionReport.iceCollectionTimeMillis =
      Date.now() - this._startTime;
    this._iceCompleted = true;
    this._rtcSession._onIceCollectionComplete(
      this._rtcSession,
      isTimeout,
      this._iceCandidates.length
    );
    if (this._iceCandidates.length > 0) {
      ...
      this.transit(
        new FailedState(this._rtcSession, RTC_ERRORS.ICE_COLLECTION_TIMEOUT)
      );
    }
  }
  _checkAndTransit() {
    if (this._iceCompleted && this._signalingConnected) {
      this.transit(
        new InviteAnswerState(this._rtcSession, this._iceCandidates)
      );
...
export class InviteAnswerState extends RTCSessionState {
  ...
  onEnter() {
    ...
    rtcSession._signalingChannel.invite(
      rtcSession._localSessionDescription.sdp,
      this._iceCandidates
    );
  }
  ...
  onSignalingFailed(e) {
    ...
    if (e.name == BusyExceptionName) {
      ...
      this.logger.error(
        "Call not found. One of the participant probably hungup.",
        e
      );
...
export class AcceptState extends RTCSessionState {
  ...
  onEnter() {
    ...
    if (!self._sdp) {
      ...
      self.transit(
        new FailedState(rtcSession, RTC_ERRORS.NO_REMOTE_ICE_CANDIDATE)
      );
      return;
    }
  ...
  onSignalingHandshaked() {
    this._rtcSession._sessionReport.handshakingTimeMillis =
      Date.now() - this._rtcSession._signallingConnectTimestamp;
...
export class TalkingState extends RTCSessionState {
  onEnter() {
    ...
    this._rtcSession._sessionReport.preTalkingTimeMillis =
      this._startTime - this._rtcSession._connectTimeStamp;
  ...
  onIceStateChange(evt) {
    var iceState = this._rtcSession._strategy.onIceStateChange(
      evt,
      this._rtcSession._pc
    );
  ...
  onPeerConnectionStateChange() {
    var peerConnectionState =
      this._rtcSession._strategy.onPeerConnectionStateChange(
        this._rtcSession._pc
      );
  ...
  onExit() {
    this._rtcSession._sessionReport.talkingTimeMillis =
      Date.now() - this._startTime;
...
export class CleanUpState extends RTCSessionState {
  onEnter() {
    ...
    this._rtcSession._sessionReport.cleanupTimeMillis =
      Date.now() - this._startTime;
    this._rtcSession._onSessionDestroyed(
      this._rtcSession,
      this._rtcSession._sessionReport
    );
  }
  get name() {
    return "CleanUpState";
...
export default class RtcSession {
  ...
  constructor(
    ...
    if (!(strategy instanceof CCPInitiationStrategyInterface)) {
      throw new Error(
        "Expected a strategy of type CCPInitiationStrategyInterface"
      );
    }
    ...
    this._localStream = window.mainLocalStreamFromAws;
  ...
  transit(nextState) {
    try {
      this._logger.info(
        (this._state ? this._state.name : "null") + " => " + nextState.name
      );
  ...
  _createSignalingChannel() {
    var signalingChannel = new RtcSignaling(
      this._callId,
      this._signalingUri,
      this._contactToken,
      this._originalLogger,
      this._signalingConnectTimeout,
      this._connectionId,
      this._wssManager
    );
  ...
  _createPeerConnection(configuration, optionalConfiguration) {
    return this._strategy._createPeerConnection(
      configuration,
  ...
  connect(pc) {
    ...
    self._pc.onconnectionstatechange = hitch(
      self,
      self._onPeerConnectionStateChange
    );
  ...
  accept() {
    throw new UnsupportedOperation(
      "accept does not go through signaling channel at this moment"
    );
  }
  hangup() {
    this._state.hangup();
  ...
  async getStats() {
    ...
    var impl = async (stream, streamType) => {
      ...
      switch (streamType) {
        ...
        default:
          throw new Error(
            "Unsupported stream type while trying to get stats: " + streamType
          );
      }
      ...
      return await Promise.all(
        tracks.map(async () => {
          ...
          return this._pc.getStats().then(function (rawStats) {
            var digestedStats = extractMediaStatsFromStats(
              timestamp,
              rawStats,
              streamType
            );
            if (!digestedStats) {
              throw new Error(
                "Failed to extract MediaRtpStats from RTCStatsReport for stream type " +
                  streamType
              );
            }
    ...
    if (this._pc && this._pc.signalingState === "stable") {
      ...
      var rttReducer = (acc, stats) => {
        if (
          ...
          (acc === null || stats.rttMilliseconds > acc)
        ) {
      ...
      var audioInputRttMilliseconds = statsResult.audio.input.reduce(
        rttReducer,
        null
      );
      var videoInputRttMilliseconds = statsResult.video.input.reduce(
        rttReducer,
        null
      );

      if (audioInputRttMilliseconds !== null) {
        statsResult.audio.output.forEach((stats) => {
          stats._rttMilliseconds = audioInputRttMilliseconds;
        });
      }

      if (videoInputRttMilliseconds !== null) {
        statsResult.video.output.forEach((stats) => {
  ...
  _detachMedia() {
    ...
    if (this._remoteAudioElement) {
      window.mainRemoteStreamFromAws = null;
  ...
  _buildMediaConstraints() {
    ...
    if (self._enableVideo) {
      ...
      if (
        Object.keys(widthConstraints).length > 0 &&
        Object.keys(heightConstraints).length > 0
      ) {
